...
A new branch for every feature/fix. Name it **feature/[name]** or **fix/[name]**.

- **Base every branch on `working-branch`, not `main`.** That is the integration
  branch for this project.
- Land work through a **pull request** into `working-branch`. Do not merge locally
  and do not push directly to `working-branch` or `main`.
- Open the PR only after the work is reviewed and the checks that exist pass.
  Opening a PR is an outward-facing action, so ask first.
- Ask to delete the branch once merged.

This overrides the default squash-merge-to-`main` flow that `/complete` assumes.
When `/complete` runs, it should stage and commit on the branch, then hand off to
a PR against `working-branch` rather than merging.
