...
A digital, solo-player adaptation of D&D Module B1, *In Search of the Unknown*.
The app takes on the Dungeon Master's role: describing rooms, resolving
examinations and searches, running module events, and tracking what the player
has discovered. Content fidelity to B1 is a core requirement.

Stack: React 19 + Vite, JavaScript (no TypeScript), plain CSS with design tokens,
npm. No backend, no database.

`docs/` holds product intent that is not safe to infer from the code, and
`src/game/*` is the game engine, which Todd owns. Read `docs/00_README.md` first,
and see the Ownership section of `blueprint/context/coding-standards.md`.
